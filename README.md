...
- price - цена товара (может быть null для безценных товаров)

#### IBuyer

Описывает данные покупателя

- payment - способ оплаты (может быть 'cash' 'card' 'null')
...
- selectedProduct — товар, выбранный для подробного просмотра
Методы:
- setProducts — сохраняет массив товаров
- getProducts — возвращает все товары
- getProductById — находит товар по его идентификатору
- setSelectedProduct — сохраняет товар для просмотра
...
Методы:
- getItems — возвращает товары из корзины
...
- hasItem — проверяет, есть ли товар в корзине
### Класс Buyer
...
- setEmail — сохраняет почту
- setPhone — сохраняет телефон
- setAddress — сохраняет адрес
- getBuyerData — возвращает все данные покупателя
- clearBuyerData — очищает все данные
- validate — проверяет заполненность полей и возвращает ошибки
- isValid — проверяет, все ли данные заполнены верно

## слой коммуникации
### Класс AppApi
Назначение: получает товары с сервера и отправляет заказы на сервер
Поля:
- api - ссылка на api адрес
Методы:
- getProducts - получает товары с сервера
- createOrders - отправляет заказы на сервер